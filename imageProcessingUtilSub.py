...
class ImageProcessing(object):
    ...
    def getPixelFeatures(image,labelled_image,file_name,cls_label,histogram_bins = np.arange(256),histogram_density = True):
        ...
        pixelFeatures = np.column_stack((file_names,cls_labels,minimums,maximums,means,medians,variances,energies,
                           ...
                           skewness,kurtosis))
        '''
        pixelFeatures = np.column_stack((file_names,cls_labels,minimums,means,medians,variances,energies,
                           entropies,tenPercentiles,nintyPercentiles,
                           interquartileRanges,meanAbsoluteDeviations,robustMeanAbsoluteDeviations,rootMeanSquareErrors,
                           skewness,kurtosis))        
    ...
    def getPixelFeatureVector(image,histogram_bins = np.arange(256),histogram_density = True):
        ...
        for i in range(4):
            ...
            pixelFeatureVector = np.column_stack((minimums,maximums,means,medians,variances,energies,
                               ...
                               skewness,kurtosis))
            '''
            pixelFeatureVector = np.column_stack((minimums,means,medians,variances,energies,
                               entropies,tenPercentiles,nintyPercentiles,
                               interquartileRanges,meanAbsoluteDeviations,robustMeanAbsoluteDeviations,rootMeanSquareErrors,
                               skewness,kurtosis))
    ...
    def getPixelFeatureVectorColumns():
        ...
        pixel_cols = []
        features = ['Minimum','Mean','Median','Variance','Energy','Entropy','TenPentile','NintyPercentile',
                                   'InterQuartileRange','MeanAbsoluteDeviation','RobustMeanAbsoluteDeviation','RootMeanSquareError',
                                   'Skewness','Kurtosis']
